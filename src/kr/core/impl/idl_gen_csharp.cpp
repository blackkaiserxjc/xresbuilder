...
private:
    ...
    {
        code_ += "[Config((int)(AppType.ClientH | AppType.ClientM | AppType.Gate | AppType.Map))]";
        code_ += fmt::format("public partial class {0}Category : ACategory<{0}>", name);
        code_ += "{";
        code_ += "}\n";
    };
    ...
    void gen_array(const Type& type, const std::string& name)
    {
        if (IsPod(type.element))
        {
          ...
          code_ += fmt::format("public List<{0}> {1} = new List<{0}>();", get_type_basic(element), name);
        } 
        else if(IsObject(type.element))
        {
          ...
          code_ += fmt::format("public List<{0}> {1} = new List<{0}>();", class_name, name);
        }
    }
