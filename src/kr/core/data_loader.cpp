...
struct HeaderField
{
    explicit HeaderField(int index = 0, const std::string &value = {},
        ...
        int end_index = 0)
        : index{index}, value{value}, type{type}, begin_index{begin_index}, end_index{end_index}
    {
    }
...
void build_row(DataTable &table, std::uint32_t index, const std::vector<std::string> &row_data,
    const std::vector<HeaderField> &fields)
{
    auto convert = [&](auto &&field) {
        ...
        auto col_size = row_data.size();
        if (boost::starts_with(field.type, "["))
        {
            std::vector<std::string> results;
            for (auto index = begin_index; index != end_index && index < col_size;
                 ++index)
            {
        ...
        {
            if (begin_index >= col_size)
            {
                throw fmt::format(
                    "out of range: row_index = {} col_index = {}, col_size = {}", index,
                    begin_index, col_size);
            }
    ...
    const std::vector<HeaderField> &fields)
{
    ...
    for (auto index = start_index; index < n_rows; index++)
    {
        ...
        if (!row.empty() && row.size() >= fields.size())
        {
            build_row(table, index, row, fields);
        }
    }
}
...
bool load_from_csv(const std::string &path, DataTable &table)
{
    ...
    {
        throw "rows less start row index.";
    }
    ...
    {
        throw "type row or value row is empty.";
    }
...
bool load_from_xlsx(const std::string &path, DataTable &table)
{
    ...
    if (whole_sheet.size() < static_cast<std::uint32_t>(HeaderRowIndex::START))
    {
        throw "rows less start row index.";
    }
...
bool DataLoader::execute(const std::string &path, DataTable &table)
{
    ...
    {
        throw fmt::format("invaild path: {}", path);
    }
