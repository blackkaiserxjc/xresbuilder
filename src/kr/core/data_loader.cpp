...
void build_row(DataTable& table, const std::vector<std::string>& row_data, const std::vector<HeaderField>& fields)
{
...
void build_rows(DataTable& table, const rapidcsv::Document& doc, const std::vector<HeaderField>& fields)
{
...
{   
    try
    {
        ...
        auto type_row = doc.GetRow<std::string>(static_cast<std::uint32_t>(HeaderRowIndex::TYPE));
        auto value_row = doc.GetRow<std::string>(static_cast<std::uint32_t>(HeaderRowIndex::VALUE));
        ...
        for (auto i = 0; i < field_index_size; i++)
        {
            ...
            auto next_index = (i == field_index_size - 1) ? size : (field_indexs[i + 1]);
