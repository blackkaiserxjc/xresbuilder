...
DataColumnCollection::DataColumnCollection(DataTable *table) : table_(table) {}

DataColumnCollection::~DataColumnCollection() { clear(); }
...
size_t DataColumnCollection::count() const { return vec_.size(); }
...
DataRow::DataRow(int id, DataTable *table) : row_id_(id), table_(table) {
  columns_ = std::make_shared<DataColumnCollection>(table);
}

DataRow::~DataRow() {}

void DataRow::remove(size_t index) { columns_->remove(index); }

DataColumnCollection &DataRow::columns() { return *columns_; }

DataColumn &DataRow::operator[](size_t index) { return (*columns_)[index]; }

DataColumn &DataRow::operator[](const std::string &name) {
  return (*columns_)[name];
}

bool DataRow::operator==(const DataRow &row) { return row_id_ == row.row_id_; }

DataRowCollection::DataRowCollection(DataTable *table) : table_(table) {}

DataRowCollection::~DataRowCollection() { clear(); }

void DataRowCollection::add(std::shared_ptr<DataRow> row) {
  rows_.emplace(row->id(), row);
}

void DataRowCollection::clear() { rows_.clear(); }

size_t DataRowCollection::count() const { return rows_.size(); }
...
DataColumnCollection &DataTable::columns() { return columns_; }

DataRowCollection &DataTable::rows() { return rows_; }
